...
const AdminTrainingReviews = () => {
    ...
    useEffect(() => {
        const init = async () => {
            try {
                const rows = await getAllTrainingReviews();
                setReviews(rows);
            } catch (error) {
    ...
    // Distinct trainings present in the reviews, for the filter dropdown.
    const trainingOptions = useMemo(() => {
        const map = new Map<string, string>();
        reviews.forEach((r) => {
            if (r.training_id) map.set(r.training_id, r.trainingName || "Untitled training");
        });
    ...
    const togglePublic = async (review: AdminTrainingReview, is_public: boolean) => {
        setReviews((prev) => prev.map((r) => (r.id === review.id ? { ...r, is_public } : r)));
        try {
            await setReviewPublic(review.id, is_public);
        } catch {
            setReviews((prev) =>
        ...
        toast({
            ...
            description: is_public
                ? "It now counts toward the training's rating."
                : "It no longer counts toward the training's rating.",
        });
    };
    ...
    const confirmDelete = async () => {
        ...
        toast({
            ...
            description: "The training's rating has been recalculated.",
        });
    };
                    ...
                    <div className="bg-card border border-brand-100 rounded-2xl p-5 md:p-6 shadow-card">
                        ...
                        <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
                            <div className="space-y-1.5">
                                ...
                                <Select value={trainingFilter} onValueChange={setTrainingFilter}>
                                    <SelectTrigger id="filter-training" className="min-h-[44px] rounded-xl">
                                        <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent>
                        ...
                        {filteredReviews.length === 0 ? (
                            <p className="rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
                                {reviews.length === 0
                                    ? "No reviews yet. They arrive after Yatris complete their trainings."
                                        ...
                                        <div className="flex flex-wrap items-start gap-3">
                                            <div className="min-w-0 flex-1 space-y-1">
                                                ...
                                                <p className="text-xs text-muted-foreground">
                                                    For {review.trainingName || "Untitled training"}
            ...
            >
                <AlertDialogContent className="rounded-2xl">
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            The review by {reviewToDelete?.name || "this Yatri"} will be gone for good and the training's rating will be recalculated. This cannot be undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
...
export default AdminTrainingReviews;
