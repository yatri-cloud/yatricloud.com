...
### Icons & logos
- **UI icons:** **Lucide only** (`lucide-react`). Never mix icon sets.
- **Brand/tech logos:** local SVGs in `public/logos/` (AWS, Azure, GoogleCloud, Kubernetes, Terraform, Docker, Ansible, Python, Linux, GitHub). Add more via Devicon → `public/logos/`. Show logos over text in marquees where recognizable.
