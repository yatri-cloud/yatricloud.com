...
- **OUTLINE BUTTON HOVER RULE (STRICT):** When hovering secondary or outline action buttons (e.g. Browse Dumps, Browse Resources, Cloud Link, Open, etc.), the hover state MUST be **FULL SOLID BRAND BLUE with white text and matching border** (`hover:bg-primary hover:text-primary-foreground hover:border-primary transition`). NEVER use pale/muted gray or washed-out light blue on button hover states.
- **Secondary / Ghost:** transparent or `--secondary`, `1px solid hsl(var(--border))`, text `--foreground`.
...
| Quick Access (launchpad) | **Slim white band under the hero** — hairline top/bottom borders, eyebrow "Where do you want to go, Yatris?", a wrap rail of rounded-full chips (Lucide icon + label) that lift on hover to every key surface (vouchers, dumps, practice tests anchor, training, events, mentorship, paths, wall of fame). NOT a card grid; pure navigation. |
| Yatris Worldwide | **White band** — same filled react-simple-maps world (equirect, muted countries) as the Achievements page inside a bordered card, brand-blue spring-in markers with looping pings on the admin-managed `community_countries` option list (seeded from REAL records only), flag chips row + honest copy ({learners} from site_stats, live country count). Map chunk + world-atlas CDN load only when the section nears the viewport (IO 600px + React.lazy). |
...
- **Card grids (catalogs):** use `grid justify-center [grid-template-columns:repeat(auto-fit,minmax(min(100%,320px),360px))]` so a **lone card centers** instead of stranding left with an empty right — this, NOT container width, is the fix for "everything's on the right side." Empty tracks collapse; many cards fill the row. Applied on Training + Events.
- **Image-less cover fallback:** any card/detail hero with no thumbnail renders a branded blue cover — `bg-gradient-to-br from-primary via-brand-600 to-brand-700`, provider label (`text-[11px] uppercase tracking-[0.2em] text-white/70`) above `certificationExamCode || courseName` in white `font-display`. Use the SAME markup on the list card and the detail hero so they match; never a generic book/placeholder icon.
- **Transactional email:** all emails wrap content in the shared `BASE_TEMPLATE` (`src/lib/email-templates.ts`) — brand-blue `#007CFF` header band + centered white card + social footer; `COLORS`/`BASE_TEMPLATE` are exported for reuse (mentorship uses them too). Admin-managed copies live in the `email_templates` DB table (keep in sync via a migration). A plain-text alternative is auto-derived server-side.
