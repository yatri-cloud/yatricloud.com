...
| Community | **Light-blue band** — kinetic statement + dotted grid + breathing glow + **Yatri Cloud logo drawn in members**: dotted cloud outline (3 lobes + flat base, computed arcs in a 200×170 box) with the cheering-Yatri figure (V arms + twin legs + head dot) inside, member avatars evenly spaced along the cloud line — NO outer circle/globe. **Signature choreography (scroll-triggered, once):** the cloud outline DRAWS itself (2.2s linear pathLength) → each avatar pops in (spring) exactly as the stroke reaches its spot (delay = 0.11 + i·0.22) → figure strokes draw in (arms→legs) → head dot springs in and emits looping radar pings → idle life: dots flow around the outline (seamless dashoffset loop, cycle = dash period 5.6) + a glowing 2-layer comet orbits the outline forever (SVG animateMotion inside a fade-in motion.g). All reduced-motion guarded (static dotted logo shown). + checkmark benefits + logo marquees |
